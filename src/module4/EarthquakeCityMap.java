...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		for (Marker marker: countryMarkers) {
			int QuakesInCountry = 0;
		    for (Marker earthquake:  quakeMarkers) {
		    	PointFeature feature = new PointFeature(earthquake.getLocation( ));
		    	if (isInCountry( feature, marker)) {
		    		QuakesInCountry++;
		    	}
		    }
		    if (QuakesInCountry>0) {
		    	countryList.add(QuakesInCountry);		    
		        System.out.println(marker.getStringProperty("name")+": "+ QuakesInCountry);
		    }
		}
		int QuakesInOcean = 0;
		for (Marker earthquake:  quakeMarkers) {
		...
		System.out.println("the number of quakes that were detected in the ocean: " + QuakesInOcean);
	}
